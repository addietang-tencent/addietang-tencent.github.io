...
| 场景 | 推荐写法 | 说明 |
|---|---|---|
| 成功 / 正常 / 开启 / 生效 | `variant="green" dot` | 真实状态，建议带 dot |
| 进行中 / 创建中 / 加载中 / 正在提醒 | `variant="blue" dot` | 真实进行态，建议带 dot |
| 失败 / 异常 / 恶意 / 加载失败 | `variant="red" dot` | 真实异常态，必须带 dot |
| 关闭 / 禁用 / 待完成 / 待处理 / 维护中 | `variant="gray" dot` | 真实状态，建议带 dot |
| 全部用户 / 公共 / 推荐 / 限免 | `variant="blue"` | 信息标签，不带 dot |
| 版本号 / 分组名 / 路径 / 数量 `+N` | `variant="gray"` | 信息标签，不带 dot |
| 未开通 / 未启用 / 未检测 | `variant="gray"` 或按业务是否为状态决定 dot | 如果只是权益/配置说明，不带 dot |
...
| 成员管理 | `/admin/members` | `client/src/pages/admin/MemberManagement.tsx` | 12 | 分组/部门摘要、角色、账号正常/禁用、配置项摘要 |
...
| 镜像管理 | `/admin/image-management` | `client/src/pages/admin/ImageManagement.tsx` | 3 | 应用范围、分组数量 |
| 模型配置 | `/admin/model-config` | `client/src/pages/admin/ModelConfig.tsx` | 4 | 全部用户、分组范围、更多数量 |
...
1. **先统一使用语义**：把 `StatusTag` 分为“状态标签”和“信息标签”。状态标签带 dot，信息标签不带 dot。
