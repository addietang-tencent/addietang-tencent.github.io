...
const adminSidebarMenuButtonVariants = cva(
  "flex h-[var(--admin-sidebar-item-height)] w-full items-center gap-2 rounded-[var(--admin-sidebar-item-radius)] px-2 text-left text-[13px] leading-5 tracking-[0.005em] text-[var(--admin-sidebar-foreground)] outline-none transition-all duration-150 hover:bg-gray-50 focus-visible:ring-2 focus-visible:ring-[var(--brand-blue)] data-[active=true]:font-medium [&>img]:size-4 [&>img]:shrink-0 [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      tone: {
        default: "",
  ...
  ({ className, style, ...props }, ref) => {
    ...
    return (
      <main
        ...
        data-state={collapsed ? "collapsed" : "expanded"}
        className={cn("min-h-screen flex-1 transition-[margin-left] duration-300", className)}
