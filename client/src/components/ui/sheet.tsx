...
function SheetContent({
  ...
  /** 是否显示遮罩层（默认 true）。设为 false 时配合 `<Sheet modal={false}>` 实现非模态侧边栏。 */
  showOverlay?: boolean;
}) {
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        className={cn(
          "bg-white data-[state=open]:animate-in data-[state=closed]:animate-out fixed z-50 flex flex-col gap-4 transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
          side === "right" &&
