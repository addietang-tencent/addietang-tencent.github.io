/**
 * 分组列公共组件 - 复用于 Agent 列表（OpenClawMonitor）与记忆空间列表（InstanceTable）
 *
 * 提供：
 *   - 工具函数：根据 creator (userId) 解析所属分组
 *   - GroupColumnFilter：列头筛选 Popover 的内容（不含外壳，调用方用 <PopoverContent> 包裹）
 *   - GroupCell：单元格渲染（部门/自定义分组徽标 + 路径 + Tooltip）
 *   - filterByCreatorGroup：分组筛选谓词
 *
 * 数据源：MemberManagement/mock 的 MOCK_GROUPS / MOCK_MANUAL_GROUPS / MOCK_USERS / MOCK_USERS_MANUAL
 * 分组层级：通过 buildGroupTree 构造，选中某分组时其所有子孙分组下的用户都会被命中
 */
...
/** 获取某 creator 对应的分组信息（OneID 模式，只返回一个） */
...
/** 获取某 creator 对应的分组信息（普通模式，只返回一个） */
...
/** 获取某 creator 所属的所有分组 id（含子孙逻辑：选中某分组时，其用户应该被命中） */
...
/** 分组筛选谓词：creator 是否属于 groupFilter 选中分组（含子孙）。groupFilter 为空表示不筛选 */
...
/** 取当前模式下的分组数据源（供调用方传入 GroupColumnFilter） */
...
// ─── 单元格 ────────────────────────────────────────────────────────────
/**
 * 分组单元格：根据 creator 渲染部门/自定义分组徽标 + 路径，hover 显示完整路径 Tooltip。
 * 找不到分组时显示 "—"。
 */
