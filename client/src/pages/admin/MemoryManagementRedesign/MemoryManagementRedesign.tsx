...
const RowActions: React.FC<RowActionsProps> = ({
  ...
  if (state === 'error') {
    return (
      <div className="flex items-center gap-2">
        <Button variant="link" size="sm" className="text-[#1447E6]">重试</Button>
  ...
  if (state === 'plugin-upgrading') {
    return <Button variant="link" size="sm" className="text-[#1447E6]">查看进度</Button>;
  }
...
export const MemoryManagementRedesign: React.FC = () => {
  ...
  return (
    <div className="page-enter pb-12">
      {/* 顶部 banner：标识这是预览页面 */}
      <div className="mb-6 flex items-start gap-2 bg-amber-50 border border-amber-100 rounded-xl px-4 py-2.5">
      ...
      <div className="mb-6">
        ...
        {!isProActive && !isProActivating && (
          <div className="bg-gradient-to-r from-purple-50 via-blue-50 to-white border border-purple-100 rounded-xl px-5 py-4 flex items-center gap-4">
            <div className="shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center">
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#FAFAFA] border-b border-[#F5F5F5]">
                ...
                <th className="text-left px-6 py-3 text-xs font-medium text-[#737373] tracking-wide" style={{ width: '26%' }}>
                  Agent 名称 / ID
                </th>
                <th className="text-left px-6 py-3 text-xs font-medium text-[#737373] tracking-wide" style={{ width: '20%' }}>
                  创建人
                </th>
                <th className="text-left px-4 py-3 text-xs font-medium text-[#737373] tracking-wide" style={{ width: '12%' }}>
                  类型
                </th>
                <th className="text-left px-6 py-3 text-xs font-medium text-[#737373] tracking-wide" style={{ width: '18%' }}>
                  记忆服务
                </th>
                <th className="text-left px-6 py-3 text-xs font-medium text-[#737373] tracking-wide" style={{ width: '24%' }}>
                  操作
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#F5F5F5]">
              {paged.length === 0 ? (
                ...
                paged.map(inst => {
                  ...
                  return (
                    <tr key={inst.id} className="hover:bg-[#FAFAFA] transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-0.5 min-w-0">
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <span className="text-sm font-medium text-[#0A0A0A] truncate max-w-[260px]">
                                {inst.name}
                              </span>
                            </TooltipTrigger>
                            <TooltipContent>{inst.name}</TooltipContent>
                          </Tooltip>
                          <span className="text-xs font-mono text-[#1447E6]">{inst.id}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm text-[#737373]">{inst.creator}</td>
                      <td className="px-4 py-4 text-xs text-[#737373]">{inst.agentType}</td>
                      <td className="px-6 py-4">
