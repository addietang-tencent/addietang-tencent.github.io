...
import { Eye, Info, PackageCheck, Settings, ShieldCheck } from "lucide-react";
  ...
  return (
    <>
      ...
      <Dialog open={openVersionConfigVisible} onOpenChange={setOpenVersionConfigVisible}>
        <DialogContent className="sm:max-w-[700px] gap-0 overflow-hidden p-0">
                ...
                <div key={item.label} className="flex items-center justify-between rounded-lg border border-gray-100 bg-white px-4 py-3">
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    {item?.apiKey === 'AutoRenew' ? Object.keys(tempOrderAutoRenewMap)?.every?.(d => tempOrderAutoRenewMap?.[d]) ?
                      <span className="text-blue-600">• 已开启</span> :
                        <span className="text-gray-400">• 未开启</span> :
                          tempAiOrderConfig?.[item?.apiKey] == 1 ? <span className="text-blue-600">• 已开启</span> :
                            <span className="text-gray-400">• 未开启</span>}
            ...
            <div className="space-y-3">
              {aiOrderList?.length ? (
                aiOrderList.map((order: any, index: number) => {
                  ...
                  return (
                    <div key={orderId} className="rounded-lg border border-gray-100 bg-gray-50/60 px-4 py-3">
                      <div className="mb-3 flex items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          />
                          <span className="text-xs text-gray-500">自动续费</span>
                          <button
                            type="button"
                      ...
                      <div className="grid grid-cols-2 gap-6 text-xs">
                        <div>
                          <div className="mb-1 text-gray-500">配额数量</div>
                          ...
                          <div className="h-1.5 overflow-hidden rounded-full bg-gray-200">
                            <div
                              className={`h-full rounded-full ${isNearFull ? 'bg-orange-400' : 'bg-blue-500'}`}
                              style={{ width: `${usagePercent}%` }}
                            />
                          </div>
                        </div>
                        <div>
                          <div className="mb-1 text-gray-500">防护有效期</div>
          ...
          <DialogFooter className="border-t px-6 py-4">
            <Button variant="outline" onClick={() => setOpenVersionConfigVisible(false)}>
              取消
            </Button>
        ...
        <DialogContent className="sm:max-w-[700px] gap-0 overflow-hidden p-0">
          ...
          {quotaDetailOrder && (
            <div className="px-8 py-7">
              <div className="mb-7 grid grid-cols-2 gap-x-1 gap-y-5">
                <div>
                  <div className="mb-2 text-sm font-medium text-gray-400">配额数量</div>
                  <div className="text-s font-semibold text-gray-800">
                    {getOrderUsed(quotaDetailOrder)} / {getOrderTotal(quotaDetailOrder)} 已使用
                  </div>
                </div>
                <div>
                  <div className="mb-2 text-sm font-medium text-gray-400">防护有效期</div>
                  <div className="text-sm font-semibold text-gray-800">
                    {getOrderStartTime(quotaDetailOrder)} ~ {getOrderEndTime(quotaDetailOrder)}
                  </div>
                </div>
                <div>
                  <div className="mb-2 text-sm font-medium text-gray-400">自动续费</div>
                  <div className={`text-s font-semibold ${getOrderAutoRenew(quotaDetailOrder) ? 'text-green-500' : 'text-gray-400'}`}>
                    {getOrderAutoRenew(quotaDetailOrder) ? '已开启' : '未开启'}
                  </div>
                </div>
                <div>
                  <div className="mb-2 text-sm font-medium text-gray-400">配额类型</div>
                  <div className="text-s font-semibold text-gray-800">AI防护版</div>
                </div>
              </div>

              <div className="mb-4 flex items-center gap-2 text-s font-semibold text-gray-700">
                <span>🤖</span>
              ...
              <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
                {bindMachinesLoading ? (
                  <div className="flex items-center justify-center px-6 py-8 text-sm text-gray-400">
                    <Spinner className="mr-2" />
                    加载中...
                  </div>
                ) : bindMachinesData.length ? (
                  bindMachinesData.map((agent: any, index: number) => (
                    <div
                      key={agent?.MachineExtraInfo?.InstanceID}
                      className="grid grid-cols-2 items-center border-b border-gray-100 px-6 py-4 last:border-b-0"
                    >
                  ...
                  <div className="px-6 py-8 text-center text-sm text-gray-400">暂无已绑定 Agent</div>
                )}
              </div>
          ...
          <DialogFooter className="border-t px-8 py-6">
            <Button variant="outline" onClick={() => setQuotaDetailOrder(null)}>
              关闭
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={openProtectModalVisible} onOpenChange={setOpenProtectModalVisible}>
        <DialogContent className="sm:max-w-[640px] gap-0 overflow-hidden p-0">
          ...
          <div className="px-7 py-6">
            <div className="mb-6 flex items-center border-b border-gray-100">
              <button
                type="button"
                className={`relative px-6 pb-3 text-sm font-medium transition-colors ${
                  protectOpenType === "new" ? "text-blue-600" : "text-gray-500 hover:text-gray-900"
                }`}
                onClick={() => setProtectOpenType("new")}
              >
                新购
                {protectOpenType === "new" && <span className="absolute bottom-[-1px] left-0 h-0.5 w-full bg-blue-600" />}
              </button>
              <button
                type="button"
                className={`relative px-6 pb-3 text-sm font-medium transition-colors ${
                  protectOpenType === "quota" ? "text-blue-600" : "text-gray-500 hover:text-gray-900"
                }`}
                onClick={() => setProtectOpenType("quota")}
              >
                使用现有配额
                {protectOpenType === "quota" && <span className="absolute bottom-[-1px] left-0 h-0.5 w-full bg-blue-600" />}
              </button>
            </div>
            ...
            {protectOpenType === "new" ? (
              <>
                  ...
                  <div className="flex gap-3">
                    {protectPeriodOptions.map(item => (
                      <button
                        key={item.value}
                        type="button"
                        className={`h-11 min-w-[96px] rounded-lg border px-6 text-sm font-medium transition-all ${
                          protectPeriod === item.value
                            ? "border-blue-500 bg-blue-50 text-blue-600 shadow-[0_0_0_1px_rgba(37,99,235,0.18)]"
                ...
                <div className="mb-5 flex items-start gap-1.5 text-xs leading-5 text-gray-500">
                  <Info className="w-4 h-4 text-blue-400 mt-0.5 shrink-0" />
                  <span>
                    <span className="font-medium text-blue-600">提示：</span>
                    开启防护后，将立即生效并开始计费。防护到期前可在「防护版本配置」中设置自动续费。
                  </span>
                </div>

                <div className="rounded-xl border border-blue-100 bg-blue-50/40 px-5 py-5">
                  <div className="mb-3 text-sm font-medium text-gray-500">预估费用</div>
                  <div className="mb-2 flex items-end gap-2">
                    <span className="text-3xl font-bold leading-none text-blue-600">¥{16*Number(selectedType === 'batch' ? selectedAgentIds?.length : 1)}</span>
                    <span className="pb-1 text-sm font-medium text-gray-500">/月</span>
                  </div>
                  <div className="text-xs text-gray-500">{selectedType === 'batch' ? `${selectedAgentIds?.length}个Agent x ¥16/月，含全量AI安全检测能力` : '单Agent防护费用，含全量AI安全检测能力'}</div>
                </div>
              </>
            ) : !aiOrderList?.length ? (
              <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50 px-5 py-8 text-center">
                <div className="mb-2 text-sm font-medium text-gray-900">暂无可用配额</div>
                <div className="text-xs text-gray-500">当前暂无可用于AI Agent安全的防护配额，请选择新购后开启防护。</div>
              </div>
            ) : (
                  ...
                  {aiOrderList.map((order: any, index: number) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        disabled={disabled}
                        className={`flex w-full items-center gap-4 rounded-xl border px-5 py-5 text-left transition-all ${
                          disabled
                            ? 'cursor-not-allowed border-gray-100 bg-white text-gray-300 opacity-90'
                            : selected
                            ? 'border-blue-600 bg-blue-50/50 shadow-[0_0_0_1px_rgba(37,99,235,0.25)]'
                      ...
                      >
                        <span
                          className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 ${
                            selected && !disabled ? 'border-blue-600' : 'border-gray-200'
                          }`}
                        >
                          {selected && !disabled && <span className="h-3 w-3 rounded-full bg-blue-600" />}
                        </span>
                        <div className="min-w-0 flex-1">
                          <div className="mb-2 flex items-center gap-3">
                            <span className={`text-base font-semibold ${disabled ? 'text-gray-300' : 'text-gray-800'}`}>
                              AI防护版配额 #{index + 1}
                            </span>
                            <span
                              className={`text-xs ${disabled ? 'text-gray-300' : 'text-blue-600 hover:text-blue-700'}`}
                              role="button"
                              tabIndex={0}
                              onClick={(e) => {
                                e.stopPropagation();
                            ...
                            {disabled ? (
                              <span className="rounded-md bg-red-50 px-2 py-0.5 text-xs font-medium text-red-400">
                          ...
                          <div className={`flex flex-wrap gap-x-7 gap-y-1 text-sm ${disabled ? 'text-gray-300' : 'text-gray-500'}`}>
          ...
          <DialogFooter className="border-t px-7 py-4">
            <Button variant="outline" onClick={() => setOpenProtectModalVisible(false)}>
              取消
            </Button>
            <Button
