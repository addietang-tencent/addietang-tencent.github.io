              ...
              <div>
                {outboundRules.length === 0 && (
                  <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-4">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-red-800">请添加出站规则，否则用户端无法正常使用openclaw</p>
                      <p className="text-xs text-red-600 mt-0.5">出站规则为空时，所有出站流量将被拒绝，导致用户端无法连接到 OpenClaw 服务</p>
                    </div>
                  </div>
                )}
