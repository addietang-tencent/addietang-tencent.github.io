// 接口端点详情数据 — 完整的 98 个接口
// 按接口分类分组，每个端点包含：描述、认证、输入参数、输出参数、请求示例、响应示例、错误码
...
const quotaEndpoints: EndpointDetail[] = [
  {
    method: "GET",
    path: "/quota/data",
    description: "查询当前用户的模型用量统计数据。支持按日期、模型、实例等维度分组聚合。",
    auth: "必须",
    inputParams: [
      ...
      { name: "group_by", type: "string", required: "否", description: "分组维度（逗号分隔），可选值：date、model、instance，默认 date,model" },
    ...
    outputParams: [
      ...
      { name: "group_by", type: "array[string]", description: "实际使用的分组维度" },
      { name: "rows", type: "array", description: "统计数据行" },
      { name: "rows[].date", type: "string", description: "日期（按 date 分组时）" },
      { name: "rows[].ai_model_id", type: "uint", description: "模型 ID（按 model 分组时）" },
      { name: "rows[].model_name", type: "string", description: "模型名称（按 model 分组时）" },
