...
2. **"应用范围"术语与交互不统一**：模型/技能已有 `ScopePopover`，通道/平台策略的"按分组"被灰掉，Agent 工具库无应用范围，镜像/VPC/安全组/EIP/记忆/网盘是"企业单例"模式。
3. **分组能力受限**：单层 + 单归属，无法表达真实企业的组织架构与跨团队协作。
4. **无法对接 OneID**：当前无法处理真实企业的组织架构、用户组、主部门/兼任等场景。
5. **无"最终生效配置"透视**：管理员无法快速判断某用户实际拥有哪些资源，及其覆盖来源。

### 0.2 本次目标

1. 将「分组」升级为**多层级树 + 多来源 + 多归属**的统一概念，支持普通模式与 OneID 模式共用同一套底层
2. OneID 模式下对接 OneID 的"组织架构"与"用户组"，组织架构需管理员显式勾选同步
3. 统一全站"应用范围"术语为 `全部用户 / 按分组`，抽出共享组件 `ScopePopover`
4. 唯一型资源（VPC/安全组/EIP/镜像/记忆/网盘/平台策略）从"企业单例"改造为"多条资源 + 应用范围 + 可选平台默认"
5. 用户管理页只保留 `全部 / 分组` 两个 tab；OneID 模式下新增「部门」列展示主部门+兼任
6. 分组视图右侧新增「配置总览」Tab，与「成员列表」并列；带初始化异常提醒
7. 分组左侧列表增加健康圆点与"仅看异常"筛选
8. 用户端分组选择（多分组时二选一）**规划写入 PRD**，**本期不做实现**
...
| 术语 | 含义 | 备注 |
|---|---|---|
| **应用范围** | 一条资源对谁生效，由"全部用户"一档 或 "按分组 `groupIds[]`"构成 | 两档互斥 |
| **全部用户** | 所有用户命中 | |
| **按分组** | 命中指定分组及其**所有后代分组**的所有成员 | 多层级树，勾父即勾子树 |
| **平台默认** | 唯一型资源的全局兜底（可选），单选 | |
| **分组** | 统一抽象概念，见 §1.2 | v2.0 合并了 v1.0 的"分组"和"部门" |
| **主部门** | OneID 返回的 `primaryDeptId`，用户的主要归属；指向一个 `source='oneid-dept'` 的分组记录 | 仅 OneID 模式有 |
...
### 1.2 「分组」的统一抽象（v2.0 核心）

**分组**是一棵带来源标记的多层级树，用 `UserGroup` 表统一表达，通过 `source` 字段区分三种来源：
...
- 用户归属是**多对多**：一个用户可以同时属于多个分组（不论来源）
- 单用户可加入的分组数**无上限**
- 分组层级深度**无上限**
- **勾选父分组 = 勾选该分组子树下的全部成员**（scope 判定时）
...
"U 属于 g 子树" = U 的 `groupIds[]` 中任一分组是 g 自身或 g 的后代。
...
| `primary-dept` | 命中分组 g 是 OneID 组织架构(`source='oneid-dept'`) 且 g 位于 U 的**主部门祖先链**上 |
| `secondary-dept` | 命中分组 g 是 OneID 组织架构(`source='oneid-dept'`) 且 g 位于 U 的**兼任祖先链**上 |
...
| 冲突类型 | 触发 | 决策入口 |
|---|---|---|
| 多分组同层冲突 | U 属于多个分组，分别被不同资源命中 | (1) 编辑资源应用范围时弹决策 Dialog；(2) 用户行 `⚠ 分组冲突 [详情]` Popover |
...
当用户归属 **≥ 2 个分组**（不论来源）时，管控端的资源计算依然按 §1.3 / §1.6 执行（多来源合并）。

**但用户端在创建 Agent 时需要知道"我这次用哪个分组的资源"**。为此设计：
- 用户端记录 `activeGroupId`（用户 profile 字段）
- 每次创建 Agent 时，按 `activeGroupId` 指向的那一条分组线去解析资源
- 触发用户端重新选择的条件见 §4（用户端规划）

> 管控端本期只做"提醒管理员：这些用户的分组名会在用户端暴露"的常驻 Alert；**用户端选择交互本期不实现**。
...
- 分组左树：OneID 模式下**新增「导入组织架构」按钮**、来源徽标
- 列展示：OneID 模式下分组列细分组织架构/用户组
...
|---|---|---|
| **全部** | — | 全宽列表（搜索 + 筛选 + 表格） |
| **分组** | 分组树（带圆点 + 筛选 + 导入按钮） | Tab 组（`成员列表 | 配置总览`） |
...
**沿用现有**：搜索、部门/分组筛选下拉、角色筛选、清除筛选按钮、表格选择、分页。
...
**普通模式**：只会有 `source='manual'` 的分组，列展示示例：

```
产品组, AI 攻坚组
```

- 多个分组逗号分隔；超过 2 个显 `前两个 +N`
- 每个分组点击跳到分组视图对应节点（可选，本期可先不做跳转）

**OneID 模式**：可能混合 `oneid-group` 和 `oneid-dept`（以及主部门本身作为部门也算一条）。展示规则：

- **分组列仅展示"用户组 + 非主部门的兼任组织架构"**（主部门已在「部门」列展示，避免重复）
- 类型用**小图标**区分：
...
**多分组提醒徽标**：如果该用户归属分组数（分组列中显示的条目数）`>= 2`，在分组列尾加一个橙色 `⚠` 小图标，HoverCard 说明"该用户在用户端需选择使用哪个分组，分组名会被暴露"（辅助说明；顶部 Alert 是主提醒，见 §3.3）。
...
```
┌────────────────────────────────────────────────────────────────┐
│ [顶部 Alert：多分组提醒，常驻]  (仅当存在归属 ≥2 分组的用户)      │
├─────────────────┬──────────────────────────────────────────────┤
│                 │  [分组名] · N 人       [Tab: 成员 | 配置]     │
│  [左侧分组树]   │  ─────────────────────────────────────       │
│                 │                                                │
...
#### 3.3.1 顶部常驻 Alert（多分组暴露提醒）

**位置**：分组 tab 的**最顶部**（左树和右内容区之上，贯穿整个内容区宽度）。

**触发**：只要系统中存在**至少一位归属 ≥ 2 个分组**的用户就展示；否则隐藏。
...
```
[⚠] 当前有 N 位用户归属了多个分组。这些用户在端侧创建 Agent 时，
     需要自行选择使用哪个分组，且分组名称会被展示给用户。
     [查看这些用户 →]
```
...
**「+ 新建分组」按钮**（仅普通模式）：品牌渐变主按钮，弹出新建分组对话框（可选择 `parentId` 作为所属父分组）。
...
- 成员数=该分组**直属成员 + 所有后代分组成员**去重合计

#### 3.3.4 右侧「成员列表」Tab

表格列与§3.2.1 全部视图一致（OneID 模式多出"部门"列），额外新增：

- **「覆盖状态」列**（仅 OneID 模式，表达该用户相对当前分组节点的覆盖情况）
- **「操作」列**：`查看配置` → 抽屉

表格数据：当前分组**及所有后代分组**的去重成员列表。
...
**这对应了确认点 5-2**：初始化检查**要**考虑该分组用户通过部门继承/全部用户档的满足情况，不是只看本节点是否直接被某条资源指向。

#### 3.3.5 右侧「配置总览」Tab（新）

展示**当前分组节点** 作为 `scope=filtered.groupIds=[节点id]` 目标时，直接指向它的所有资源；并**做初始化检查**。
...
├──────────────────────────────────────────────────────┤
│ [模型]                                                │
│  · Claude 3.5 Sonnet   (直接配到本分组)   →模型管理   │
│  · GPT-4o              (配到祖先分组"产品部"继承得到) │
│                                                       │
│ [通道]                                                │
│  · 飞书                (直接配到本分组)    →通道管理  │
│                                                       │
...
- 不再有"按分组 vs 按部门"的二分，统一都是「按分组」
- OneID 模式下在 UI 上做**分类显示**（组织架构 / 用户组 / 自建分组），但写回的都是 `groupIds[]`
...
|---|---|---|
| `all` | `全部用户` | 灰胶囊 |
| filtered，1 个 | `{分组名}` | 蓝色胶囊 |
| filtered，多个 | `{首个分组} +N` | 蓝色胶囊 |
...
**触发**：OneID 模式，分组视图左树工具栏 `[⬇ 导入组织架构]`。

**作用**：管理员选择要把 OneID 的哪些组织架构节点同步到本系统作为 `source='oneid-dept'` 的分组。用户组 `oneid-group` **不需要此弹窗（自动同步）**。
...
- 右端 `[已同步]` 徽标表示该节点当前已是 `oneid-dept` 分组
...
- 用户归属分组数 = 0：无影响，只按"全部用户"档获得资源
- 用户归属分组数 = 1：`activeGroupId` 自动设为该分组 id，**全程无感**
- 用户归属分组数 ≥ 2：需用户显式选择（见 §4.3）

### 4.3 触发用户选择

**首次登录**：
- 登录完成后，若归属分组 ≥ 2 且 `activeGroupId` 为空 → 弹出分组选择弹窗，必选一项才能进主界面
...
| 归属分组数 1 → 2（新增加入一个）| **弹** |
| 归属分组数 2 → 3（再加一个）| **弹** |
| 归属分组数 2 → 1（移除一个，且剩的那个是 activeGroupId）| 不弹，自动把 `activeGroupId` 锁定 |
| 归属分组数 2 → 1（移除一个，但剩的那个**不是** activeGroupId）| 不弹（因为只剩一个，自动锁定） |
| 归属分组数 ≥ 2，其中一个被移除，且被移除的就是 activeGroupId | **弹**（要求重选） |
| 归属分组数 ≥ 2，原 activeGroupId 仍在 | 不弹 |
...
用户创建 Agent / 拉取可用资源时：
- `activeGroupId = null` → 按"未在任何分组"逻辑走，只拿全部用户档 + 平台默认
- `activeGroupId` 指向某分组 g → 把 g 视为"当前用户的唯一分组"做资源计算（其他分组忽略）

> 这意味着：**同一用户在不同 active 分组下看到的可用资源不同**。符合"用户自选分组"的直觉。
...
// 统一的「分组」表（v2.0 合并 v1.0 的 Department 和 UserGroup）
interface UserGroup {
  id: string;
  name: string;
  description?: string;
...
function resolveAdditive(U: User): Resource[] {
  ...
  // 注意：命中条件是"U 的某归属分组是 R.scope.groupIds 中某项的后代或自身"
...
来源判定（§1.4）：
- 命中分组 g 的 `source === 'oneid-dept'` 且 g 在 U 主部门祖先链上 → `primary-dept`
- 命中分组 g 的 `source === 'oneid-dept'` 且 g 在 U 兼任祖先链上 → `secondary-dept`
...
2. 用户管理页：下掉部门 tab，全部/分组列调整
3. 分组视图：左树多层级 + 圆点 + 筛选；右 Tab（成员列表 / 配置总览）
4. OneID 导入组织架构弹窗
5. 多分组暴露常驻 Alert
...
- I1 "分组+部门可同时勾" → 废弃（不再有部门 scope）
- G4 "镜像保留 agentType 分组" → 保持
- G6 "全部用户 ≠ 平台默认" → 保持
